...
- **Sync edits** (`pushSessionEditsNow` in `services/decks.ts`, global banner `ui/SyncEditsButton.tsx`) — pushes only the cards created/edited this browser session (tracked in-memory by `lib/sync/sessionEdits.ts`, populated by `saveCard`), unconditionally overwriting whatever's remote — no merge, no conflict check. Only appears once there's something to push.
