...
Every mutation writes to Dexie first and is immediately usable offline. Two push paths: a 30s-debounced full resync as a safety net, and a fire-and-forget single-doc push for near-immediate propagation. `runFullSync` (session start, gated by `SyncContext`/`ReviewSyncGate`) does: pull a remote snapshot → apply tombstones (deletions win if newer than the entity) → per-entity conflict resolution by `updatedAt` (real conflicts surface to the user via `SyncConflictModal`) → media sync by content digest (only differing blobs move) → push anything local-only or local-newer. Writes during merge re-check tombstone/staleness inside the same Dexie transaction to close races against concurrent user edits mid-sync.
